...
namespace OperatorsOverloading
{
    ...
    public abstract class List<TValue>
    {
        ...
        public static bool operator ==(List<TValue> list1, List<TValue> list2)
        {
            ...
            var comparsionList1 = list1.ToFlat().ToList();
            var comparsionList2 = list2.ToFlat().ToList();

            return comparsionList1.SequenceEqual(comparsionList2);
        }
        ...
        public static List<TValue> operator -(List<TValue> list1, List<TValue> list2)
        {
            return List.From(list1.ToFlat().Where(x => !list2.ToFlat().ToList().Contains(x)));
        }
        ...
        {
            for (var curr = this; !curr.IsNil; curr = curr.Tail) yield return curr;
        }
        ...
        {
            return "[ " + string.Join(", ", Flatten().Select(l => l.Head)) + " ]";
        }
    ...
    public static class List
    {
        ...
        public static List<TItem> From<TItem>(IEnumerable<TItem> items)
        {
            ...
            var curr = Of(elems.First());

            foreach (var e in elems.Skip(1)) curr = Of(e, curr);
        ...
        {
            return From(Enumerable.Repeat(item1, 1).Concat(items));
        }
